...
#AreaSD
set.seed(259)
AreaSD_hours <- rnorm(100, mean = 90, sd = 3)
AreaSD_Income <-  0.25*abs(AreaSD_hours * (rnorm(100, 105,5))) +700
...
#AreaLA
set.seed(259)
AreaLA_hours <- rnorm(90, mean = 92, sd = 3)
AreaLA_Income <-  0.25*abs(AreaLA_hours * (rnorm(90, 97,6))) +500
...
#AreaB
set.seed(259)
AreaB_hours <- rnorm(110, mean = 93, sd = 3)
AreaB_Income <- 0.25*abs(AreaB_hours * (rnorm(110, 90,6))) +100
...
#Irvine
set.seed(259)
AreaI_hours <- rnorm(80, mean = 93, sd = 4)
AreaI_Income <-  0.25*abs(AreaI_hours * (rnorm(80, 73,5)))
...
#Riverside
set.seed(259)
AreaR_hours <- rnorm(75, mean = 96, sd = 3)
AreaR_Income <-  0.25*abs(AreaR_hours * (rnorm(65, 68,4))) -300
...
p <- ggplot(data=All, aes(x=Hours, y=Income)) + geom_point()+geom_point(color="pink")+geom_smooth(method = "lm", se = FALSE, color = "lightblue")
p<- p + theme_classic()
